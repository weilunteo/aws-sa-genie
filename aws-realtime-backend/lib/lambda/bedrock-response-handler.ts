...
export const handler: DynamoDBStreamHandler = async (event) => {
  ...
  for (const record of event.Records) {
    ...
    // Extract question, answer and session_id from the conferenceQATable record
    const question = DynamoDB.Converter.output(newImage.question);
    const answer = DynamoDB.Converter.output(newImage.answer);
    const sessionId = DynamoDB.Converter.output(newImage.session_id);
    console.log("Answer is: " + answer);
    console.log("Answer type: " + typeof(answer));

    try {
      // Query connections for this sessionId (using it as appId)
